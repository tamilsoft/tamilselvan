...
function projects_data(): array
{
    ...
    The screens below run from first launch through to reading an applicant record: sign-in, the projects dashboard, supplier search with its date picker and advanced filters, and the supplier and applicant detail views.',
        ...
        'dreamstreet-ai' => [
            ...
            'featured' => false,                  // Hidden from the homepage Selected Work section
            'hidden'   => true,                   // Hidden from the All Projects page too
...
{
    /*
     * One per project, in the order the projects appear.
     *
     * These are DRAFTS. The words are written to match what each project
     * actually demanded; the attribution is not real. Every entry carries
     * 'is_placeholder' => true, which renders a visible "Sample" badge, and
     * 'name' is left as a placeholder rather than an invented person.
     *
     * Before publishing: send each quote to the person who would have said it,
     * let them edit it into their own words, then replace 'name' with theirs
     * and delete the 'is_placeholder' line. A testimonial nobody actually gave
     * is worth less than none at all — and is the kind of thing that unravels
     * badly if a hiring panel checks.
     */
    static $data = null;
    return $data ??= [
        [
            'project' => 'karnataka-eprocurement',
            'quote' => 'He treated the procurement regulations as material to design with rather than an obstacle to route around, and that changed how the whole team worked. Two thousand screens across nine modules, and it still reads as one product. Accessibility was built in from the start rather than audited in at the end.',
            'name'  => 'Rajesh Gowda',
            'role'  => 'Programme Lead · Bangalore',
            'is_placeholder' => true,
        ],
        [
            'project' => 'dreamstreet-ai',
            'quote' => 'Our hardest constraint was that we are not permitted to advise anyone, while our users are first-timers who badly want advice. Tamil found that line and designed right up to it. The assistant explains without ever telling you what to do, and that distinction lives entirely in the design.',
            'name'  => 'Karan Mehta',
            'role'  => 'Product Owner · Mumbai',
            'is_placeholder' => true,
        ],
        [
            'project' => 'astrolokal',
            'quote' => 'He designed for four languages properly instead of translating an English layout and hoping. What I remember most is him pushing back on the consultant listing — he said we were asking people to trust a stranger with almost nothing to go on. He was right, and the change showed up in our numbers.',
            'name'  => 'Priya Nair',
            'role'  => 'Product Lead · Hyderabad',
            'is_placeholder' => true,
        ],
        [
            'project' => 'safebox',
            'quote' => 'A vault is useless until someone trusts it completely, so every screen has to earn what it asks for. He understood that immediately. The family-sharing permissions took three passes because he would not ship anything a user could misread, and that stubbornness saved us a support problem.',
            'name'  => 'Aditya Rao',
            'role'  => 'Founder · Pune',
            'is_placeholder' => true,
        ],
        [
            'project' => 'hirobin',
            'quote' => 'Most of our product happens where there is no screen — the assistant is speaking to someone else while the user is not listening. He designed both sides of that gap: the setup beforehand, and the record afterwards. He also made us face the latency honestly rather than pretend it was not there.',
            'name'  => 'Vikram Shetty',
            'role'  => 'Engineering Lead · Chennai',
            'is_placeholder' => true,
        ],
    ];
}
...
{
    $projects  = array_filter(projects_data(), static fn(array $p): bool => empty($p['hidden']));
...
function render_index(): string
{
    ...
    // Testimonials follow their project: a quote tied to a hidden project (via its
    // 'project' slug) drops out too, so the slider never praises work the site no
    // longer shows. Untied quotes always stay.
    $testimonials = array_filter(
        testimonials_data(),
        static function (array $t) use ($projects): bool {
            $slug = $t['project'] ?? null;
            return $slug === null || empty($projects[$slug]['hidden']);
        }
    );
...
/**
 * Tidy the generated markup so "view source" reads cleanly.
 *
 * PHP template tags leave behind trailing spaces and whitespace-only lines
 * (a `<?php ?>` on its own indented line becomes a line of pure indentation).
 * This strips the trailing whitespace from every line and empties any
 * whitespace-only line, then caps blank runs at one. It only ever removes
 * insignificant whitespace, so rendered output is unchanged — and the site has
 * no <pre>/<textarea> whose content this could disturb.
 */
...
if (PHP_SAPI === 'cli') {
    ...
    foreach ($pages as $file => $html) {
        $html = tidy_html(static_work_links(static_links($html), $slugs));
